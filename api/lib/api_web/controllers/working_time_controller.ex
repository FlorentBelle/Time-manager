defmodule ApiWeb.WorkingTimeController do
  ...
  def create(conn, params) do
    ...
    if (user !== nil) do
      ...
      conn |> render(ApiWeb.WorkingTimesView, "post_working_time.json", %{status: 201, success: true, message: "Workingtime registered"})
    else
  ...
  def retrieveAll(conn, params) do
    ...
    if ((start_ === nil || end_ === nil) && userID !== nil) do
      if (start_ !== nil) do
        ...
        conn |> render(ApiWeb.WorkingTimesView, "get_working_time.json", %{status: 200, success: true, message: "Workingtime retrieved for start query parameter", content: retrieved})
      else
        if (end_ !== nil) do
          ...
          retrieved = Repo.all(from w in WorkingTimes, where: w.user == ^userID, where: w.end <= ^end__)
            conn |> render(ApiWeb.WorkingTimesView, "get_working_time.json", %{status: 200, success: true, message: "Workingtime retrieved for end query parameter", content: retrieved})
        end
      end
    else
      if(userID !== nil) do
    ...
    if (is_integer(userID) &&  userID !== nil) do
      ...
      if(user !== nil) do
        ...
        if(workingtime !== nil) do
          conn |> render(ApiWeb.WorkingTimesView, "get_working_time.json", %{status: 200, success: true, message: "Working time retrieved for the user", content: workingtime})
        else
    ...
    if(userID !== nil && id !== nil && is_integer(userID) && is_integer(id)) do
      ...
      if(user !== nil) do
        if(working_time !== nil) do
          conn |> render(ApiWeb.WorkingTimesView, "get_working_time.json", %{status: 200, success: true, message: "One working time found for this user", content: working_time})
        else
  ...
  def update(conn, params) do
    id = params["id"]
    userID = String.to_integer(params["user"])

    if(id !== nil) do
      id = String.to_integer(params["id"])
      working_time = Repo.one(from w in WorkingTimes, where: w.user == ^userID, where: w.id == ^id)
      if(working_time !== nil) do
        working_time
        |> WorkingTimes.changeset(params)
        |> Repo.update()
        conn |> render(ApiWeb.WorkingTimesView, "get_working_time.json", %{status: 200, success: true, message: "Working time updated for this user", content: params})
      else
        conn |> render(ApiWeb.ErrorView, "error.json", status: 403, error: "No working time found")
      end
    else
  ...
  def delete(conn, params) do
    ...
    if(id !== nil) do
      ...
      if(working_time !== nil) do
        ...
        conn |> render(ApiWeb.WorkingTimesView, "post_working_time.json", %{status: 204, success: true, message: "Working time deleted"})
      else
        conn |> render(ApiWeb.ErrorView, "error.json", status: 403, error: "No working time found")
      end
    else
